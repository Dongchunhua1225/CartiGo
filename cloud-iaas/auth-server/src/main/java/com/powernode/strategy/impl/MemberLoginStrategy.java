...
public class MemberLoginStrategy implements LoginStrategy {
    ...
    public UserDetails realLogin(String username) {
        ...
        //调用get方法调用登录凭证校验接口 -> hutool的方法
        String jsonStr = HttpUtil.get(url);

        //判断响应是否有值
        if(!StringUtils.hasText(jsonStr)) {
            throw new InternalAuthenticationServiceException("登陆异常，请重试");
        }

        //将登录凭证校验接口响应的json格式字符串，转换为json对象 -> fastjson
        ...
        if(ObjectUtil.isNull(loginMember)) {
            //没有这个会员 -> 注册
                //创建会员对象到我们的微信小程序的用户体系内
            loginMember = registerMember(openid);
        }
        //存在 ->1.判断会员账号的状态
        if(!loginMember.getStatus().equals(1)) {
            //会员状态不正常
            throw new InternalAuthenticationServiceException("帐号状态异常，请联系平台工作人员");
        }

        //2. 返回Security框架能够识别的安全用户对象 SecurityUser
