...
public class NoticeServiceImpl extends ServiceImpl<NoticeMapper, Notice> implements NoticeService{
    ...
    @Override
    @Caching(evict = {
            @CacheEvict(key = StoreConstants.WX_TOP_NOTICE_KEY),
            @CacheEvict(key = StoreConstants.WX_ALL_NOTICE_KEY)
    })
    ...
    @Override
    @Cacheable(key = StoreConstants.WX_TOP_NOTICE_KEY)
    public List<Notice> queryTopWXNoticeList() {
    ...
    @Override
    @Cacheable(key = StoreConstants.WX_ALL_NOTICE_KEY)
    public List<Notice> queryWXAllNoticeNotice() {
        return noticeMapper.selectList(new LambdaQueryWrapper<Notice>()
                .eq(Notice::getStatus,1)
                .orderByDesc(Notice::getCreateTime));
    }
